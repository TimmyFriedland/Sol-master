...
/** Constants */
#define kMIN_TIME_SINCE_UPDATE          360000
#define kMAX_NUM_WEATHER_VIEWS          500
...
@interface SOLMainViewController ()<MKMapViewDelegate>
...
@property(strong, nonatomic) UIButton* showMap;
...
- (void)initializeSubviews
{
    ...
    self.pageControl = [[UIPageControl alloc]initWithFrame: CGRectMake(0, CGRectGetHeight(self.view.bounds) - 52,
    ...
    _showMap = [[UIButton alloc]initWithFrame:CGRectMake(SCREEN.size.width/2, SCREEN.size.height-20, 20, 30)];
    [_showMap setBackgroundColor:[UIColor greenColor]];
    
    [_showMap addTarget:self
...
//        if(weatherView.local == NO) {
            ...
            if([[NSDate date]timeIntervalSinceDate:weatherData.timestamp] >= 0 || !weatherView.hasData) {
...
- (void)updateWeatherView:(SOLWeatherView *)weatherView withData:(SOLWeatherData *)data
{
    ...
    if(data.forecastSnapshots.count>1)
    {
